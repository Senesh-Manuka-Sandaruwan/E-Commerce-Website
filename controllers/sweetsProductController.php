...
if (isset($_POST['add_product'])) {
   ...
   if (empty($product_name) || empty($sub_product_name) || empty($product_price) || empty($product_image)) {
       ...
       if (move_uploaded_file($product_image_tmp_name, $product_image_folder)) {
           $insert = "INSERT INTO sweetsproducts (product_name, sub_product_name, product_price, product_image) VALUES ('$product_name', '$sub_product_name', '$product_price', '$product_image')";
...
if (isset($_GET['delete'])) {
   ...
   $query = mysqli_query($conn, "SELECT product_image FROM sweetsproducts WHERE id = $id");
   $row = mysqli_fetch_assoc($query);
   $image_file = 'uploaded_img/' . $row['product_image'];

   // Delete the product from the database
   $delete_query = "DELETE FROM sweetsproducts WHERE id = $id";
   if (mysqli_query($conn, $delete_query)) {
       // Delete the image file
       if (file_exists($image_file)) {
           unlink($image_file);
       }
       header('location:sweetsProductController.php');
   } else {
...
<div class="container mx-auto p-4">
   ...
   $select = mysqli_query($conn, "SELECT * FROM sweetsproducts");
         ...
         <tbody>
            ...
            <tr class="hover:bg-gray-50">
               ...
               <td class="border border-gray-300 px-4 py-2 space-x-2">
                  <a href="sweetsProductController.php?delete=<?php echo $row['id']; ?>" 
                     class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Delete</a>
                  <a href="updateSweetsProduct.php?edit=<?php echo $row['id']; ?>" 
