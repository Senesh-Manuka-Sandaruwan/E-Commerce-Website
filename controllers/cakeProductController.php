...
if (isset($_POST['add_product'])) {
   ...
   if (empty($product_name) || empty($sub_product_name) || empty($product_price) || empty($product_image)) {
      ...
      if (move_uploaded_file($product_image_tmp_name, $product_image_folder)) {
         $insert = "INSERT INTO cakeproducts (product_name, sub_product_name, product_price, product_image) VALUES ('$product_name', '$sub_product_name', '$product_price', '$product_image')";
...
if (isset($_GET['delete'])) {
   ...
   $query = mysqli_query($conn, "SELECT product_image FROM cakeproducts WHERE id = $id");
   $row = mysqli_fetch_assoc($query);
   $image_file = 'uploaded_img/' . $row['product_image'];

   // Delete the product from the database
   $delete_query = "DELETE FROM cakeproducts WHERE id = $id";
...
<?php include '../header.php'; ?>
   ...
   $select = mysqli_query($conn, "SELECT * FROM cakeproducts");
